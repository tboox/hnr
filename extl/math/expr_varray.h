 ...
 * Updated:		08.08.14
 *
 * Brief: expression varray class - delay to calculate value array expression
 *
 * [<Home>]
...
#define EXTL_MATH_EXPR_VARRAY_H

#ifndef EXTL_MATH_EXPR_VARRAY_SUPPORT
#	error expr_varray.h is not supported by the current compiler.
#endif

/*!\file expr_varray.h
 * \brief expression varray class
 */
...
#include "detail\varray_expr.h"

#ifdef EXTL_MATH_VARRAY_TEST_ENABLE
#	include "..\counter\counter.h"
#	include "basic_varray.h"
#endif
/* ///////////////////////////////////////////////////////////////////////
 * ::extl namespace 
 */
...
class expr_varray
{
	...
	public:
		...
		typedef expr_varray															class_type;
	...
	/// @{
	public:
		explicit_k expr_varray(array_type& ar)
			: m_array(ar)
		{}
	/// @}

	private:
		expr_varray(class_type const& rhs);
	...
	public:
		...
		template<	typename_param_k T2
				,	typename_param_k E2
				>
		class_type& operator =(EXTL_NS_DETAIL(varray_expr)<T2, E2> const& expr)
		{
...
template<	typename_param_k T
		...
		,	typename_param_k A2
		>
EXTL_NS_DETAIL(varray_expr)	<	T
							,	EXTL_NS_DETAIL(eop_add)	<T, A1, A2>
							>
operator +(expr_varray<T, A1> const& ar1, expr_varray<T, A2> const& ar2)
{
	return EXTL_NS_DETAIL(varray_expr)	<	T
										,	EXTL_NS_DETAIL(eop_add)	<T, A1, A2>
										>
										(	EXTL_NS_DETAIL(eop_add)	<T, A1, A2>
																	(	ar1.array()
...
EXTL_NS_DETAIL(varray_expr)	<	T
							...
							>
operator +(expr_varray<T, A1> const& ar1, EXTL_NS_DETAIL(varray_expr)<T, E2> const& expr2)
{
	return EXTL_NS_DETAIL(varray_expr)	<	T
										,	EXTL_NS_DETAIL(eop_add)	<T, A1, E2>
										>
...
EXTL_NS_DETAIL(varray_expr)	<	T
							...
							>
operator +(EXTL_NS_DETAIL(varray_expr)<T, E1> const& expr1, expr_varray<T, A2> const& ar2)
{
	return EXTL_NS_DETAIL(varray_expr)	<	T
										,	EXTL_NS_DETAIL(eop_add)	<T,	E1,	A2>
										>
...
EXTL_NS_DETAIL(varray_expr)	<	T
							,	EXTL_NS_DETAIL(eop_add)	<	T
														...
														> 
							>
operator +(expr_varray<T, A1> const& ar1, T const& value)
{
	return EXTL_NS_DETAIL(varray_expr)	<	T
										,	EXTL_NS_DETAIL(eop_add)	<	T
																	,	A1
...
EXTL_NS_DETAIL(varray_expr)	<	T
							,	EXTL_NS_DETAIL(eop_add)	<	T
														...
														> 
							>
operator +(T const& value, expr_varray<T, A2> const& ar2)
{
	return EXTL_NS_DETAIL(varray_expr)	<	T
										,	EXTL_NS_DETAIL(eop_add)	<	T
																	,	EXTL_NS_DETAIL(eop_scalar)<T>
