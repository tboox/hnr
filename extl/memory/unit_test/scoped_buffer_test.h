 ...
 * Updated:		08.04.14	
 *
 * Brief: scoped_buffer class unit testing
 *
 * [<Home>]
...
#	error This file must be included of scoped_buffer.h
#endif
...
static int scoped_buffer_test()
{
	...
	counter.end();
	EXTL_TEST_TRACE(_T("scoped_buffer<>::resize(0 - 256B) time:%d ms"), counter.ms());
	...
	counter.end();
	EXTL_TEST_TRACE(_T("scoped_buffer<>::resize(0 - 1024B) time:%d ms"), counter.ms());
	...
	counter.end();
	EXTL_TEST_TRACE(_T("scoped_buffer<>::resize(0 - 8K) time:%d ms\n"), counter.ms());
	...
	counter.end();
	EXTL_TEST_TRACE(_T("vector<>::resize(0 - 256B) time:%d ms"), counter.ms());
	...
	counter.end();
	EXTL_TEST_TRACE(_T("vector<>::resize(0 - 1024B) time:%d ms"), counter.ms());
	...
	counter.end();
	EXTL_TEST_TRACE(_T("vector<>::resize(0 - 8K) time:%d ms\n"), counter.ms());
#endif
	buffer_type buf2;
	counter.begin();
	for(i=0; i < 100000; i++)
